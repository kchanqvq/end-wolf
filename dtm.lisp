...
(defvar *endgames* (make-array 18 :initial-element nil))
...
(defun wolf-forward-scan-init-7 (sheep wolf)
  (declare (tablebase sheep wolf))
  (sb-thread:barrier (:memory))
  (assert (= (length sheep) (length wolf) (max-state 7)))
  (format t "~&Init scan, 7 pieces wolf-to-move.~%")
  (do-states (state sm wm 7) ()
    (when (zerop (aref wolf state))
      (let ((fate +sheep-m1+))
        (declare (fate fate))
        (wolf-forward sm wm (lambda (next-sm next-wm)
                              (declare (ignore next-sm next-wm))
                              (setq fate (max fate 0)))
                      (lambda (next-sm next-wm)
                        (declare (ignore next-sm next-wm))
                        (setq fate (1- +wolf-m1+))))
        (setf (aref wolf state) fate)))))

(defun wolf-forward-scan-init (sheep wolf prev-sheep n-piece)
  ...
  (format t "~&Init scan, ~a pieces wolf-to-move.~%" n-piece)
  (do-states (state sm wm n-piece) ()
    (when (zerop (aref wolf state))
      (let ((fate +sheep-m1+))
        (declare (fate fate))
...
(defun wolf-forward-scan (sheep wolf n-piece)
  (declare (tablebase sheep wolf))
  (sb-thread:barrier (:memory))
  (assert (= (length sheep) (length wolf) (max-state n-piece)))
  ...
  (do-states (state sm wm n-piece) ()
    (when (zerop (aref wolf state))
      (let ((fate +sheep-m1+))
        (declare (fate fate))
        (wolf-move-forward sm wm (wolf-updater fate sheep n-piece))
        (setf (aref wolf state) fate)))))
...
(defun wolf-forward-scan-create-worklist (sheep wolf n-piece)
  (declare (tablebase sheep wolf))
  (sb-thread:barrier (:memory))
  (assert (= (length sheep) (length wolf) (max-state n-piece)))
...
(defun wolf-forward-scan-use-worklist (sheep wolf worklist n-piece)
  (declare (tablebase sheep wolf))
  (sb-thread:barrier (:memory))
...
(defun init (n-piece)
  (unless (aref *endgames* n-piece)
    (setf (aref *endgames* n-piece) (cons nil nil)))
...
(defun run (n-piece)
  (assert (<= 7 n-piece 17))
  (unless (or (= n-piece 7) (car (aref *endgames* (1- n-piece))))
  ...
  (iter (with sheep = (car (aref *endgames* n-piece)))
    ...
    (if (first-iteration-p)
        (if (= n-piece 7)
            (wolf-forward-scan-init-7 sheep wolf)
            (wolf-forward-scan-init sheep wolf prev-sheep n-piece))
        (wolf-forward-scan sheep wolf n-piece))
...
(defun run-worklist (n-piece)
  ...
  (iter (with sheep = (car (aref *endgames* n-piece)))
    ...
    (with processed = 0)
    (for worklist
         initially (wolf-forward-scan-create-worklist sheep wolf n-piece)
         then (wolf-forward-scan-use-worklist sheep wolf worklist n-piece))
    (while worklist)
...
#+nil (defun load-endgames (n-piece sheep-file wolf-file)
...
#+nil (defun save-endgames (sheep-file wolf-file)
