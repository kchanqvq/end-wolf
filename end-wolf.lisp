...
(defun reencode-wolf-index (mask state)
  (declare (optimize speed)
           (type board mask)
           (type state state))
  (let ((bitpos 0) (logcount 0)
        (which 0) (a 0) (b 0) (c 0))
    (declare (type (unsigned-byte 5) logcount))
    (dotimes (i 5)
      (dotimes (j 5)
        (when (plusp (ldb (byte 1 bitpos) state))
          (when (plusp (ldb (byte 1 bitpos) mask))
            (ecase which
              (0 (setf a logcount))
              (1 (setf b logcount))
              (2 (setf c logcount)))
...
(defun decode-wolf-index (state)
  ...
  (multiple-value-bind (a b c) (decode-set (ldb (byte 10 25) state))
    (let ((bitpos 0) (logcount 0) (mask 0))
      (declare (type (unsigned-byte 5) logcount)
               (type board mask))
      (dotimes (i 5)
        (dotimes (j 5)
